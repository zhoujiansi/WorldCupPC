...
export default {
  ...
  methods:{
    ...
    async sendIssue (userId) { //发送优惠券
      const parm = {
        userId: userId
        // userId: "29836823"
      }
      let re = await getIssue(parm)
      let data = re.data
      console.log("sendIssue:",re,data)
      this.isLoading=false
      if(data.success === true){
        // 发放成功
        this.showConfirm = true;
        setStorage("status","done");
      }
      else {
        if(data.resultCode === "E2005"){
          // 次数超限，已经领取过 ,直接跳转到购买页
          setStorage("status","done");
          this.$router.push('/Teacher');
        }
        else {
          alert("系统错误,请联系您的投顾,"+data.resultMessage)
        }
      }
    },
  },
